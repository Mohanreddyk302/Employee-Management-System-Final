...
//		Project project =projectService.findProjectById(projectId);
		...
		createEmployee.setAge(Period.between(employee.getDateOfBirth(), LocalDate.now()).getYears());
		...
		createEmployee.setBranchName(employee.getBranchName());
		...
		createEmployee.setPinCode(employee.getPinCode());
		createEmployee.setEmpId(employee.getEmpId());
		createEmployee.setCompanyName(employee.getCompanyName());
		createEmployee.setCompanyEmail(employee.getCompanyEmail());
		createEmployee.setManagerName(employee.getManagerName());
		createEmployee.setDateOfJoining(employee.getDateOfJoining());
		createEmployee.setOfficePhone(employee.getOfficePhone());
...
//		createEmployee.setStartDate(LocalDate.now());
//		createEmployee.setEndDate(LocalDate.now().plusMonths(1));
...
//		newEmployee.setPassword(passwordEncoder.encode(employee.getPassword()));
	...
	@Override
	public Employee updateFinanceDetails(Employee employee, Long id) throws Exception {
		Optional<Employee> optEmployee = employeeRepository.findById(id);
		if (optEmployee.isEmpty()) {
			throw new Exception("Employee not found with id " + id);
		}
		...
		//Finance Details
		newEmployee.setPanNo(employee.getPanNo());
		newEmployee.setAdhaar(employee.getAdhaar());
		newEmployee.setBankName(employee.getBankName());
		newEmployee.setAccountNumber(employee.getAccountNumber());
		newEmployee.setIfscCode(employee.getIfscCode());
		newEmployee.setBranchName(employee.getBranchName());
	...
	@Override
	public Employee updateProfessionalDetails(Employee employee, Long id) throws Exception {
		Optional<Employee> optEmployee = employeeRepository.findById(id);
		if (optEmployee.isEmpty()) {
			throw new Exception("Employee not found with id " + id);
		}
		...
		//Finance Details
		newEmployee.setCompanyName(employee.getCompanyName());
		newEmployee.setCompanyEmail(employee.getCompanyEmail());
		...
		newEmployee.setCompanyLocation(employee.getCompanyLocation());
...
//        }
	...
	@Override
	public List<Project> getPreviousProjectsByEmployeeId(Long employeeId) throws Exception {
		Employee employee = findEmployeeById(employeeId);
        if (employee == null) {
        ...
        List<Long> previousProjectIds = employee.getDates().stream()
                .filter(ed -> ed.getEmpEndDate().isBefore(now))
                .map(EmployeeDate::getProjectId)
                .distinct()
                .collect(Collectors.toList());

        return projectRepository.findAllById(previousProjectIds);
	}

	@Override
	public List<Project> getCurrentProjectsByEmployeeId(Long employeeId) throws Exception {
		Employee employee = findEmployeeById(employeeId);
        if (employee == null) {
            throw new Exception("Employee not found with id: " + employeeId);
        }
		LocalDate now = LocalDate.now();
        List<Long> currentProjectIds = employee.getDates().stream()
                .filter(ed -> !ed.getEmpEndDate().isBefore(now))
                .map(EmployeeDate::getProjectId)
                .distinct()
                .collect(Collectors.toList());

        return projectRepository.findAllById(currentProjectIds);
	}
